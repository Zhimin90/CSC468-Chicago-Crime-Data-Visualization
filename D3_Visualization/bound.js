function runD3(geojson) {
  ...
  d3.json(bound_url, function (err, data) {
    ...
    dispatchSelected.on("selected", function (data) {
      //get clientHeight
      padding = screen.height / 8;
      var h = screen.height / 3 - padding / 2;
      //console.log("h: ", h);
      ...
      let clickedWard = geojson.features.filter(
        (val) => val.properties.ward === data.properties.ward
      );
      ...
      d3.select("div.crosstabchart");

      console.log("crosstabchart height: ", Math.trunc(h + padding / 2));
    ...
    constructor(geojson) {
      ...
      function reRenderBoundaries() {
        features.forEach((feature) => {
          pathNODES[indexSorted[+feature.properties.ward]].setAttribute(
            "d",
            path.projection(getD3())(feature)
          );
        });
      }
      ...
      mapLayer
        .selectAll("path.map-layer-bound")
        .data(features)
        .enter()
        .append("path")
      ...
      geojson.features.forEach((feature) => {
        for (let i = 0; i < pathNODES.length; i++) {
          //regular expression
          if (
            feature.properties.ward === /ward([0-9]+)/g.exec(pathNODES[i].id)[1]
          ) {
  ...
  function mousemove(d) {
    Tooltip.html("<h3>Ward: " + d.properties.ward + "</h3>")
  ...
  function mouseOut(d) {
    ...
    if (c.classed("clicked")) {
      ...
      d3.select(this).attr("fill-opacity", "0.2").style("fill", baseColor);
    }
  }
  ...
  function syncHighlight(ward) {
    ...
    d3.select("#lc".concat(ward)).style("stroke-width", 10).style("opacity", 1);
  ...
  function syncHighlightRemove(ward) {
    ...
    d3.select("#lc".concat(ward))
      ...
      .style("opacity", 1);
    d3.selectAll("#ct".concat(ward)).style("stroke", "none");
    d3.selectAll("#rc".concat(ward)).style("stroke", "none");
  } //syncHighlightRemove
}
